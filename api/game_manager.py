...
# Three independent serial handles (venue bring-up from hardware/com6-com7-bringup):
_hw_led_control = None   # led.led_control MODULE, COM9 emitters — write only
_hw_layout_type = 0
_hw_ready = False
_hw_wall_ctrl = None      # LedControl() INSTANCE, COM7 wall buttons — read+write
_hw_wall_ready = False
_hw_recv_ctrl = None      # LedControl() INSTANCE, COM6 laser receivers — read only
_hw_recv_ready = False

# COM6 beam-break polarity: raw True = dark. Intact lit beam → False tread.
# Do not invert (first playtest drained life when inverted).
...
def _hw_init():
    """Soft three-port open: COM9 emit (module), COM7 wall + COM6 recv (instances)."""
...
class GameInstance:
    ...
    def apply_floor_input(self, row: int, col: int, action: str):
        ...
        When real hardware is live (COM6/COM7), only touch the clicked cell —
        do not wipe HW-derived tread/wall state."""
        if not self.accepting_input or self.current_state.get("phase") != "playing":
            return False
        if self.led_table is None:
            return False
